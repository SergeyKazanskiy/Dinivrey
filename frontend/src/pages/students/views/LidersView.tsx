...
export const LidersView: React.FC = () => {
  const { liders, test, tests } = useStore();
  const { selectTest, selectStudent } = useStore();
      ...
      <Box mt={1}>
        ...
        <Box h='538px' overflow='scroll'>    
          {liders.map((lider, inx) => (
            <HStack key={inx} m={1} align='center' cursor='pointer' py={1}
              onClick={() => selectStudent(lider.id)} bg='white'>
